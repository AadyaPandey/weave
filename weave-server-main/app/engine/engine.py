...
def _has_cycle(node_ids: list[str], edges: list[dict[str, Any]]) -> bool:
    """Return True if the graph contains a cycle (DFS-based detection)."""
    ...
    return any(dfs(nid) for nid in node_ids if nid not in visited)
...
async def execute_node(
    ...
    for attempt in range(retries + 1):
        node_run_id = await asyncio.to_thread(
            db.create_node_run, run_id, node["id"], node["type"], input_data
        )
        try:
            result = await asyncio.wait_for(
                node_class().execute(input_data, config, context), timeout=timeout
            )
            await asyncio.to_thread(db.finish_node_run, node_run_id, "COMPLETED", result)
            return result
        except Exception as exc:
            await asyncio.to_thread(db.finish_node_run, node_run_id, "FAILED", error=str(exc))
            if attempt == retries:
                raise
...
async def execute(workflow: dict[str, Any], input_data: dict[str, Any]) -> tuple[int, Any]:
    ...
    run_id = await asyncio.to_thread(db.create_run, workflow["id"], input_data)
    context: dict[str, Any] = {
        "input": input_data,
        "nodes": {},
        "variables": {},
    ...
    try:
        ...
        await asyncio.to_thread(db.finish_run, run_id, "COMPLETED", last_output)
        return run_id, last_output
    except Exception as exc:
        await asyncio.to_thread(db.finish_run, run_id, "FAILED", error=str(exc))
        raise
